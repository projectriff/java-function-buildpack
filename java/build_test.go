...
func testBuild(t *testing.T, context spec.G, it spec.S) {
	...
	it("contributes invoker", func() {
		...
		Expect(result.Processes).To(ContainElements(
			libcnb.Process{Type: "java-function", Command: "streaming-http-adapter", Arguments: []string{"java", "-cp", `"${CLASSPATH}"`, "${JAVA_OPTS}", "org.springframework.boot.loader.JarLauncher"}},
			libcnb.Process{Type: "function", Command: "streaming-http-adapter", Arguments: []string{"java", "-cp", `"${CLASSPATH}"`, "${JAVA_OPTS}", "org.springframework.boot.loader.JarLauncher"}},
			libcnb.Process{Type: "web", Command: "streaming-http-adapter", Arguments: []string{"java", "-cp", `"${CLASSPATH}"`, "${JAVA_OPTS}", "org.springframework.boot.loader.JarLauncher"}},
		))
	})
